...
C.append(io("跑完整的落库前 audit。**它是一个常驻脚本，这里只是调用它** —— "
            "逻辑只有一份，笔记本和命令行跑的是同一套。",
            "`../flowgt-job-hunter/jobs.db` + `audit-local/audit.json` + 一次 dry-run",
            "七段检查，每段都有【在验什么 / 实测数字 / PASS·WARN·FAIL / 这意味着什么】",
            "有 FAIL 就不要推 —— 脚本最后会告诉你原因"))
C.append(code(
...
"| WARN | 为什么留着不修 |",
"|---|---|",
"| `job_type` / `jd` 永远为空 | `job_type` 没人用；`jd` 要接上 `fetch_jd()`，那是另一件事 |",
"| `Featured` 岗位解析不出时间 | **这是对的设计** —— 比起把它们当成「刚刚」发出去要好 |",
))
